...
void main(void) {
	...
	//vec3 true_norm = mat3(inverseMV) * vec3(norm);
	...
	if (shadow_coeff != 1.0) {
		...
		gl_FragColor = texture2D(u_texture, v_texcoord) + vec4(total_ambient + total_diffuse + total_specular, 1.0);
}
